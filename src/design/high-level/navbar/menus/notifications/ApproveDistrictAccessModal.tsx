import { Dialog, FormControl, MenuItem, Select, SelectChangeEvent, Typography } from "@mui/material";
...
interface IApproveDistrictAccessModal {
  handleDone: (role: XLogsRole) => void;
...
export function ApproveDistrictAccessModal(props: IApproveDistrictAccessModal) {
  ...
  const [selectedRole, setSelectedRole] = useState<XLogsRole>("Service Provider - Autonomous");

  function onSelectChange(event: SelectChangeEvent<XLogsRole>) {
    const role = event.target.value;
    setSelectedRole(role as XLogsRole);
  }
  return (
    <Dialog
      className="noselect"
    ...
    >
      ...
      >
        ...
        <Typography variant="body1">Please select the appropriate role for the provider you would like to approve.</Typography>
        <Typography variant="body1" sx={{ fontWeight: "bold" }}>{requestingUser?.firstName} {requestingUser?.lastName}:</Typography>
        <Box
          display="inline-flex"
          sx={{
            gap: getSizing(1)
        ...
        >
          <Typography variant="body1" sx={{ textDecoration: "underline" }}>Service Provider Type:</Typography>
          <Typography variant="body1">Coming soon{/* TODO: Add service provider type to district access request */}</Typography>
        </Box>
        <Box
          display="inline-flex"
          sx={{
        ...
        >
          <Typography variant="body1" sx={{ textDecoration: "underline" }}>X Logs Role Assigned:</Typography>
